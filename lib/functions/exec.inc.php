...
//------------------criados por mim para incrementar as funcionalidades do editExecution----------------------------------------------------------

//função que traz do banco os dados dos steps para o editExecution.tpl não sei se é ideal deixar isso aqui mas como quase todas as funções desse arquivo legado fazem acesso ao banco, achei apropriado.
function get_execution_steps(&$dbHandler,$execution_id){
	$tables = tlObjectWithDB::getDBTables(array('executions','execution_tcsteps','tcsteps'));
	$sql="select b.* , c.step_number,c.actions,c.expected_results,c.active,c.execution_type from {$tables['executions']} as a inner join {$tables['execution_tcsteps']} as b on (a.id = b.execution_id) inner join {$tables['tcsteps']} as c on (b.tcstep_id = c.id) WHERE a.id =  ".intval($execution_id);//."foçarndo erro";//strval($execution_id);
	$rs = $dbHandler->get_recordset($sql);
	return $rs;
}
// função que faz o update dos steps no banco os dados para o editExecution.tpl 
function updateExecutionNotesSteps(&$db,$request){
	$key2loop = array_keys($request['step_notes']);//essa linha me permite pegar os IDs dos steps que serão alterados pois o índice do vetor é o id na tabela também.
	foreach($key2loop as &$step_id){
		$sql = "UPDATE execution_tcsteps".//não usei a notação presente nas outras queries desse arquivo para pegar os prefixos e sufixos das tabelas pois aparentemente aquilo não funciona com essa tabela. depois preciso investigar se é isso mesmo ou se fui ei que tentei implementar da maneira errada para essa tabela.
		" SET notes = ?, status = ? where id = ?";
		$db->db->execute($sql, array($request['step_notes'][$step_id],$request['step_status'][$step_id],$step_id));
	}    
  //return $db->exec_query($sql)/*executa a querie*/ ? tl::OK : tl::ERROR;   //não interpretei o que essa linha faz. Mas como ela era presente em quase todas as execuções de queries, achei mais sábio simplesmente não tirar  
}
//altera o status da execução
function updateStatus(&$db,$status,$id){
	$db->exec_query("update executions set status = '".$status."' where id = ".$id);
	
}


//------------------criados por mim para que sejam gerados os execution_tcsteps na execução por bulk----------------------------------------------------------

function generateBlankSteps(&$db,$status,$id,$execid){
	$rs = $db->get_recordset("select id from nodes_hierarchy where parent_id = ".$id);var_dump( $rs);
	foreach($rs as $step_id){
		$db->exec_query("insert into execution_tcsteps(execution_id,tcstep_id,notes,status) values(".$execid.",'".$step_id['id']."','','".$status."')");
	}
