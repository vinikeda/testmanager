...
class issues {
	...
	function create($description,$categoryId,$whoAccept,$markers = null){
		if(!$this->verifyDuplicity($description,$markers)){
		
			$author = ($whoAccept == "QA"?'qa':'analis');
			$this->db->exec_query("insert into issues(description,category_id,".$author."_accept) values ('$description',$categoryId,1)");
			if($markers != null){
        ...
        function update ($id ,$description,$category, $markers = null){
            $sql = "update issues set description = '$description', category_id = $category where id = $id";
