...
class tlUser extends tlDBObject
{
  ...
  */
  ...
  function getAccessibleTestPlans(&$db,$testprojectID,$testplanID=null, $options=null)
  {
    ...
    $sql = " /* $debugTag */  SELECT distinct {$fields2get} " .
           ...
           " AND USER_TPLAN_ROLES.user_id = " . intval($this->dbID) . " left join builds on (nh.id = builds.testplan_id)".
           " WHERE testproject_id = " . intval($testprojectID) . " AND ";
    
    
    if (!is_null($my['options']['active'])) 
    {
      $sql .= " TPLAN.active = {$my['options']['active']} AND ";
    }
    ...
    // new dBug($sql);  
    $sql .= " ORDER BY builds.is_open desc, name";//echo $$my['options']['output'].PHP_EOL.PHP_EOL;
    $numericIndex = false;
...
function getAccessibleTestPlansFilteringBySubadiq(&$db,$testprojectID,$subadiq,$options=null)
  {
    ...
    $sql = " /* $debugTag */  SELECT distinct {$fields2get} " .
           ...
           " ON TPLAN.id = USER_TPLAN_ROLES.testplan_id " .
           " AND USER_TPLAN_ROLES.user_id = " . intval($this->dbID) . " left join builds on (nh.id = builds.testplan_id)".
           " WHERE testproject_id = " . intval($testprojectID) . " AND tplan.id_subadiquirentes = ".$subadiq." AND";
