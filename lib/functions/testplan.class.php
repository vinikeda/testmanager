...
class testplan extends tlObjectWithAttachments
{
   ...
   *     if problems -> 0.
   */
  function create($name,$notes,$testproject_id,$subadiq_id,$is_active=1,$is_public=1)
  {
    ...
    $sql = "/* $debugMsg */ " . 
           " INSERT INTO {$this->tables['testplans']} (id,notes,api_key,testproject_id,active,is_public,id_subadiquirentes) " .
           " VALUES ( {$tplan_id} " . ", '" . $this->db->prepare_string($notes) . "'," .
           "'" .  $this->db->prepare_string($api_key) . "'," .
           $testproject_id . "," . $active_status . "," . $public_status . " , ".$subadiq_id.")";
...
>>>>>>> tcg
  ...
  */
  function html_custom_field_inputs($id,$tproject_id,$scope='design',$name_suffix='',$input_values=null) 
  {
    $itemSet='';
    $method_suffix = $scope=='design' ? $scope : 'execution';
    $method_name = "get_linked_cfields_at_{$method_suffix}";//print_r($this->$method_name($id,$tproject_id));
    $cf_map=$this->$method_name($id,$tproject_id);
    if(!is_null($cf_map))
    {
