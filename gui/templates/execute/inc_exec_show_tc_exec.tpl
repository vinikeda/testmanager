<!-- comeca aqui o inc_exec_show_tc_exec.tpl -->
{* 
TestLink Open Source Project - http://testlink.sourceforge.net/
@filesource	inc_exec_show_tc_exec.tpl
@internal revisions
@since 1.9.13 $step_info
*}	
 ...
 <script>
var updatestat = function(elemen){
	if(elemen.value != 'p'&&elemen.value != 'b'&&elemen.value != 'n'&&document.getElementById('statusSingle_{$tcversion_id}').value != 'f')document.getElementById('statusSingle_{$tcversion_id}').value = elemen.value;
	
}
var init = new Date().getTime();
var getExecTime = function(){	
	document.getElementById('execution_duration').value = Math.round((new Date().getTime()-init)/60000)+1;
}
</script>
  ...
  {$exec_build_title="$build_title $title_sep $my_build_name"}
    ...
    {if $gui->other_execs.$tcversion_id}
      ...
      {if $gui->history_on == 0 && $show_current_build}
   		   <div class="exec_history_title">
  			    {$labels.last_execution} [teste]{$labels.exec_current_build}
...
<!-- termina aqui o inc_exec_show_tc_exec.tpl -->