...
switch($args->do_action)
{
  ...
  case 'do_create':
    $op = doCreate($args,$subadiq_mgr/*,$tplan_mgr,$date_format_cfg*/);
    //$of->Value = $op->notes;
    $templateCfg->template = $op->template;
  break;
...
function renderGui(&$smartyObj,&$argsObj,&$subadiq_mgr,$templateCfg/*,$owebeditor*/,&$guiObj)
{
    ...
    if($doRender)
    {
      ...
      $guiObj->subadiq=$subadiq_mgr->get_subadiqs($argsObj->testprojectID);
      ...
      $guiObj->subadiq_id = $argsObj->sub_adquirenteID;
...
function init_source_build_selector(&$testplan_mgr, &$argsObj) 
{
  ...
  $htmlMenu['items'] = $testplan_mgr->get_subadiqs($argsObj->testprojectID);/*$testplan_mgr->get_builds_for_html_options($argsObj->tplan_id,null,null,
