...
<h1 class="title">Sub-adquirentes{*$labels.title_build_2}{$smarty.const.TITLE_SEP_TYPE3}{$labels.test_plan}{$smarty.const.TITLE_SEP}{$gui->tplan_name|escape*}</h1>
  ...
  <form method="post" id="buildView" name="buildView" action="{$managerURL}">
    ...
    {* table id MUST BE item_view to use show/hide API info *}
  	<table id="item_view" class="simple_tableruler sortable">
  		...
  		{foreach item=build from=$gui->subadiq}
        	<tr>
          ...
          <td class="clickable_icon">
            {if $build.active==1} 
                <input type="image" style="border:none" id="set_build_active"
                       ...
                       onClick = "do_action.value='setInactive';sub_adquirenteID.value={$build.id};"
                       src="{$tlImages.on}"/>
              {else}
                <input type="image" style="border:none" id="set_build_inactive"
                     ...
                     onClick = "do_action.value='setActive';sub_adquirenteID.value={$build.id};"
                     src="{$tlImages.off}"/>
              {/if}
          </td>
          ...
          <td class="clickable_icon">
            {if $build.is_open==1} 
                <input type="image" style="border:none" id="close_build"
                       ...
                       onClick = "do_action.value='close';sub_adquirenteID.value={$build.id};"
              ...
              {else}
                <input type="image" style="border:none" id="open_build"
                     ...
                     onClick = "do_action.value='open';sub_adquirenteID.value={$build.id};"
                     src="{$tlImages.lock}"/>
              {/if}
          </td>
